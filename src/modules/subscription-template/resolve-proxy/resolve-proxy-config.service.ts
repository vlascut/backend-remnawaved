...
export class ResolveProxyConfigService {
    ...
    private resolveEarlyExitRemarks(
        ...
        if (hostCount === 0) {
            if (settings.customRemarks.emptyHosts.length > 0) {
                return settings.customRemarks.emptyHosts;
            }

            if (user.activeInternalSquads.length !== 0) {
                return ['→ Enterlance VPN', 'В подписке нет серверов', '→ No hosts found'];
            }

            return ['→ Enterlance VPN', 'В подписке нет сквадов', '→ No internal squads found'];
        }
